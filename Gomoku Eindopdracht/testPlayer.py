...
STATIC_MATRIX_2 = getMatrix(2)
def get_adj_moves(state, move):
    global STATIC_MATRIX_2
    ...
    # always loops over a static amount of adjacent moves; len(static_matrix_3)
    for m in STATIC_MATRIX_2:
        ...
        if all_moves:
            if adj:
                ...
                for move in all_moves:
                    # print('move: ', move, ' w ->  ', get_adj_moves(state, move))
...
class testPlayer:
    ...
    def move(self, state, last_move, max_time_to_move = 1000):
        ...
        # if state[1] >= 7:
            # MEANS, if white in this case played 3 stones next to each other, we would lose in 2 moves, so we need to rpevent that
            # on move 7, we haven't finished out opening
            # but we need to add enemy moves, so less change of using out opening moves...

        if not self.valid_moves: 
            if not self.opening_done:
                self.opening_done = True
        
            self.valid_moves = get_valid_moves(state, True)
        else:

            if self.opening_done:

                self.valid_moves.extend(  get_adj_moves(state, last_move))
                self.valid_moves = remove_duplicates(self.valid_moves)
        ...
        while ((time.time() - start_time) * 1000) < max_time_to_move:  
            ...
            # counter += 1
        # print('did: ', counter, ' loops!')
        # n_root.printNode()
        # print(best_uct_child(n_root).printNode())
...
def FindSpotToExpand(_node):
    ...
    if len(_node.children) != len(_node.valid_moves):
        ...
        n_child.valid_moves = copy.copy(_node.valid_moves)
